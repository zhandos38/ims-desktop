<template>
    <div class="app-container">
        ...
        <div class="row">
            <table class="table table-bordered">
                ...
                >
                    ...
                    <td :class="{'text-danger': ((product.diff = parseFloat(product.currentQty) - parseFloat(product.quantity)) < 0), 'text-success': (product.diff > 0)}">{{ product.diff }}</td>
                    <td>{{ product.price_in }}</td>
                    <td>{{ Math.abs(((product.price_in / product.price_retail) - 1) * 100).toFixed(2) }}%</td>
                    <td>{{ product.price_retail }}</td>
                    <td :class="{'text-danger': ((product.diffAmount = (product.diff * parseFloat(product.price_retail))) < 0), 'text-success': product.diffAmount > 0}">{{ product.diffAmount }}</td>
                    <td
        ...
        <div class="row">
            <button class="btn btn-outline-success w-100" @click="save">Сохранить</button>
        </div>
    </div>
</template>
